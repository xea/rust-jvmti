...
#[link(name = "cvoid")]
extern "C" {
    fn C_dostuff(jvmtiPtr: *mut jvmtiEnv) -> ();
}
...
pub extern fn R_Agent_OnLoad(vm: *mut JavaVM, options: *mut ::libc::c_char, reserved: *mut ::libc::c_void) -> jint {
...
#[no_mangle]
#[allow(non_snake_case)]
pub extern fn R_test_call(jvmtiPtr: *mut jvmtiEnv) -> () {
    unsafe {
        println!("RUST Test jmtpPtr: {:p} to {:p}", jvmtiPtr, *jvmtiPtr);
        let fnptr = (**jvmtiPtr).AddCapabilities.unwrap();
        println!("RUST Capptr {:p}", fnptr);

        let caps = jvmtiCapabilities {
            _bindgen_bitfield_1_: 0,
            _bindgen_bitfield_2_: 0,
            _bindgen_bitfield_3_: 0,
            _bindgen_bitfield_4_: 0
        };

        let result = fnptr(jvmtiPtr, &caps);
...
pub extern fn Agent_OnLoad(vm: *mut JavaVM, options: *mut ::libc::c_char, reserved: *mut ::libc::c_void) -> jint {
    ...
    unsafe {
        ...
        let penvPtr: *mut *mut c_void = &mut voidPtr as *mut *mut c_void; // <- this has been verified as correct conversion
